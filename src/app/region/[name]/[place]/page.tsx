...
export default function PlaceMapPage({ params: { place } }: { params: { place: string } }) {
    ...
    const handleRouteSave = () => {
        ...
        axios
            .post(
                `${process.env.NEXT_PUBLIC_LOCAL_URL}/api/content/${decodeURIComponent(place)}`,
    ...
    useEffect(() => {
        if (!place) return;
        axios
            .get(`${process.env.NEXT_PUBLIC_LOCAL_URL}/api/content/${decodeURIComponent(place)}`, {
            ...
            <div className="w-full h-full min-w-[1920px] flex flex-col items-center">
                ...
                <div className="w-full flex flex-col gap-[11px] pb-[36px] mt-5 items-center">
                    <div className=" px-[14px] py-[10px] border border-[#D0D5DD] rounded-[8px]">
                        <p className="text-[#667085] text-[24px] font-bold">서울여행:{decodeURIComponent(place)}</p>
                    </div>
                    <div className="w-full flex items-center justify-center gap-4">
                        {contentPlace?.map((placeName, index) => {
                            ...
                            return (
                                <>
                                    <PlaceCard region={decodeURIComponent(place)} placeName={placeName.placeName} />
